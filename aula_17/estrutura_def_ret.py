...
def exibir_cardapio():
    ...
    print("Margheritta - P: R$25 | M: R$35 | G: R$45")
    print("Calabresa - P: R$28 | M: R$38 | G: R$48")
    print("Frango - P: R$30 | M: R$40 | G: R$50")

# exibir_cardapio()

# Função para aplicar desconto, onde o preço e o percentual de desconte será passado no momento da invocação da função.
valor_sem_desconto = 40

def aplicar_desconto(preco, percentual):
    # preco * (1 - percentual /100)
    return preco * percentual

preco_final = valor_sem_desconto - aplicar_desconto(valor_sem_desconto, 0.10)
...
def fazer_pedido(sabor, tamanho="M", borda_recheada = False):
    ...
    # variável = valor se verdadeiro if condição logico else valor se falso
    print(f"Pedido: {sabor} | {tamanho} | {borda}")

fazer_pedido("Margheritta")
fazer_pedido("Frango", "G")
fazer_pedido("Margheritta", "P", True)
